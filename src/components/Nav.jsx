...
<Link className={`last:pr-24 cursor-pointer capitalize transition duration-100 transform hover:scale-125 hover:text-white active:text-red-500 ${ pathname.includes(`/home/${key}`) || genre === key ? 'text-red-500' : ''}`} href={`/home/${key}`} key={key} onClick={() => handleChangeGenre(key)}>{title}
  </Link>
))}
</ul>
</nav>
